...
from bot.database.db_requests import  get_steam_urls_by_userid, set_steam_urls_for_user, get_all_steam_urls

router = Router()

@router.callback_query(lambda query: query.data == "inventory")
async def handle_inventory(query: CallbackQuery):
    user_accs = await get_steam_urls_by_userid(query.from_user.id)
    if(user_accs is not None):
        await utils.edit_message_media(query, 'RISE_FOR_FARMERS', get_inventory_kb(), caption=f"Ваши аккаунты: ")
...
async def handle_add_accounts(query: CallbackQuery, state: FSMContext):
    message_id = await query.message.edit_caption(caption='Введите пажэ список инвентарей.\nСписок можно передать в формате steamid64 или ссылками на профиль.', reply_markup=get_inventory_settings_kb())
...
async def handle_accounts_statistics(query: CallbackQuery):
    all_accounts = await get_all_steam_urls()
...
async def process_inventory_list(message: Message, state: FSMContext):
    ...
    try:
        ...
        msg = utils.process_lines_multithread(lines)
      
        await message.delete()
        await bot.edit_message_caption(chat_id=message.chat.id, message_id=message_id, reply_markup=get_inventory_kb(), caption=f"Ваши аккаунты: {msg}")
        await set_steam_urls_for_user(message.from_user.id, msg)
